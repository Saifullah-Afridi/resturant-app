...
const DishesAdmin = () => {
  ...
  const fetchCategoriesList = async () => {
    try {
      ...
      const filteredList = res.data.categories.map(category => category.name)
  ...
  return (
    <>
      ...
      {openModal && <AddDishModal categoriesList={categoriesList} openModal={openModal} setOpenModal={setOpenModal} refetchDishes={fetchDishes} />}

      {dishes ?
        <DishesAdminTable refetchDishes={fetchDishes} categoriesList={categoriesList} dishes={dishes} />
        : <div>
          <h1>loading.....</h1>
        </div>
